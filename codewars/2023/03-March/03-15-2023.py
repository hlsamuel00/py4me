...
def closest(string: str) -> list[int]:
    results, str_list = [], string.split()
    
    for idx, num in enumerate(str_list):
        sum_of_digits = sum(int(digit) for digit in num)
        results.append([[sum_of_digits, idx, int(num)]])
    
    if len(results) < 2: return results
    
    results.sort()
    min_idx = min((results[i][0] - results[i - 1][0], i) for i in range(1, len(results)))[1]
    
    return results[min_idx - 1: min_idx + 1]

# This was the solution I liked the most:
